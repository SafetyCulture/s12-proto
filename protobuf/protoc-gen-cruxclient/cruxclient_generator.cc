...
inline string StringReplace(string str, const string &from, const string &to, bool replace_all)
{
  size_t pos = 0;
...
void PrintHeaderPrologue(Printer *printer, const FileDescriptor *file)
{
  ...
  printer->Print(vars, "// Generated by the CRUX Service Client C++ plugin.\n");
  printer->Print(vars, "// If you make any local change, they will be lost.\n");
  printer->Print(vars, "// source: $filename$\n");
  printer->Print(vars, "#ifndef CRUX_CLIENT_$filename_identifier$__INCLUDED\n");
  printer->Print(vars, "#define CRUX_CLIENT_$filename_identifier$__INCLUDED\n");
  printer->Print(vars, "\n");
}
...
void PrintHeaderInterfaces(Printer *printer, const FileDescriptor *file)
{
  ...
  for (int service_index = 0; service_index < file->service_count(); ++service_index)
  {
    ...
    for (int method_index = 0; method_index < service->method_count(); ++method_index)
    {
      ...
      {
        printer->Print(vars, "virtual $response$ $method_name$(const $request$& request) const = 0;\n");
      }
    }
...
public:
  ...
  bool Generate(const FileDescriptor *file, const string &parameter, GeneratorContext *context, string *error) const override
  {
    ...
    std::unique_ptr<ZeroCopyOutputStream> header_output(context->Open(file_name + ".crux.client.pb.h"));
    ...
    std::unique_ptr<ZeroCopyOutputStream> source_output(context->Open(file_name + ".crux.client.pb.cc"));
