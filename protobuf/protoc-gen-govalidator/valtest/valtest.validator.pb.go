...
func (m *LogOnlyValidationMessage) Validate() error {
	...
	if m.Answer != "" {
		var _len_LogOnlyValidationMessage_Answer = utf8.RuneCountInString(m.Answer)
		if _len_LogOnlyValidationMessage_Answer > 40 || _len_LogOnlyValidationMessage_Answer < 0 {
			fmt.Printf("[log-only] %s: value must be %s: Base64Encoded input: %s\n", "answer", "have a length between 0 and 40", proto.Base64Encode(proto.FirstCharactersFromString(fmt.Sprintf("%v", m.Answer), 50)))
		}
	}
	if m.Note != "" {
		var _len_LogOnlyValidationMessage_Note = utf8.RuneCountInString(m.Note)
		if _len_LogOnlyValidationMessage_Note > 40 || _len_LogOnlyValidationMessage_Note < 5 {
...
func (m *SimpleStringMessage) Validate() error {
	var _len_SimpleStringMessage_SubjectToken = utf8.RuneCountInString(m.SubjectToken)
	if _len_SimpleStringMessage_SubjectToken > 8192 || _len_SimpleStringMessage_SubjectToken < 1 {
		return fmt.Errorf(`subject_token: value must have a length between 1 and 8192`)
	}
	var _len_SimpleStringMessage_Audience = utf8.RuneCountInString(m.Audience)
	if _len_SimpleStringMessage_Audience > 253 || _len_SimpleStringMessage_Audience < 1 {
		return fmt.Errorf(`audience: value must have a length between 1 and 253`)
	}
	if m.Note != "" {
		var _len_SimpleStringMessage_Note = utf8.RuneCountInString(m.Note)
		if _len_SimpleStringMessage_Note > 40 || _len_SimpleStringMessage_Note < 5 {
			return fmt.Errorf(`note: value must have a length between 5 and 40`)
		}
	}
	return nil
