...
func (m *AuthCallbackRequest) Validate() error {
	if m.RawState != "" {
		if !norm.NFC.IsNormalString(m.RawState) && norm.NFD.IsNormalString(m.RawState) {
			// normalise NFD to NFC string
			var normErr error
			m.RawState, _, normErr = transform.String(transform.Chain(norm.NFD, norm.NFC), m.RawState)
			if normErr != nil {
				return fmt.Errorf(`raw_state: value must must be normalisable to NFC`)
			}
		}
		if strings.ContainsRune(m.RawState, utf8.RuneError) {
			return fmt.Errorf(`raw_state: value must must have valid encoding`)
		} else if !utf8.ValidString(m.RawState) {
			return fmt.Errorf(`raw_state: value must must be a valid UTF-8-encoded string`)
		}
		var _len_AuthCallbackRequest_RawState = len(m.RawState)
		if !(_len_AuthCallbackRequest_RawState >= 50 && _len_AuthCallbackRequest_RawState <= 500) {
			return fmt.Errorf(`raw_state: value must have a length between 50 and 500`)
		}
		if !_regex_49c3af0ba56ba5365f6aa218eec1b288.MatchString(m.RawState) {
			return fmt.Errorf(`raw_state: value must only have valid characters`)
		}
	}
	if m.ErrorMsg != "" {
		if !norm.NFC.IsNormalString(m.ErrorMsg) && norm.NFD.IsNormalString(m.ErrorMsg) {
			// normalise NFD to NFC string
			var normErr error
			m.ErrorMsg, _, normErr = transform.String(transform.Chain(norm.NFD, norm.NFC), m.ErrorMsg)
			if normErr != nil {
				return fmt.Errorf(`error_msg: value must must be normalisable to NFC`)
			}
		}
		if strings.ContainsRune(m.ErrorMsg, utf8.RuneError) {
			return fmt.Errorf(`error_msg: value must must have valid encoding`)
		} else if !utf8.ValidString(m.ErrorMsg) {
			return fmt.Errorf(`error_msg: value must must be a valid UTF-8-encoded string`)
		}
		var _len_AuthCallbackRequest_ErrorMsg = len(m.ErrorMsg)
		if !(_len_AuthCallbackRequest_ErrorMsg >= 1 && _len_AuthCallbackRequest_ErrorMsg <= 100) {
			return fmt.Errorf(`error_msg: value must have a length between 1 and 100`)
		}
		if !_regex_d4db71516b8749dc594e5bf604c6a110.MatchString(m.ErrorMsg) {
			return fmt.Errorf(`error_msg: value must only have valid characters`)
		}
	}
	if m.TrueClientIp != "" {
		if !norm.NFC.IsNormalString(m.TrueClientIp) && norm.NFD.IsNormalString(m.TrueClientIp) {
			// normalise NFD to NFC string
			var normErr error
			m.TrueClientIp, _, normErr = transform.String(transform.Chain(norm.NFD, norm.NFC), m.TrueClientIp)
			if normErr != nil {
				return fmt.Errorf(`true_client_ip: value must must be normalisable to NFC`)
			}
		}
		if strings.ContainsRune(m.TrueClientIp, utf8.RuneError) {
			return fmt.Errorf(`true_client_ip: value must must have valid encoding`)
		} else if !utf8.ValidString(m.TrueClientIp) {
			return fmt.Errorf(`true_client_ip: value must must be a valid UTF-8-encoded string`)
		}
		var _len_AuthCallbackRequest_TrueClientIp = len(m.TrueClientIp)
		if !(_len_AuthCallbackRequest_TrueClientIp >= 7 && _len_AuthCallbackRequest_TrueClientIp <= 50) {
			return fmt.Errorf(`true_client_ip: value must have a length between 7 and 50`)
		}
		if !_regex_d4db71516b8749dc594e5bf604c6a110.MatchString(m.TrueClientIp) {
			return fmt.Errorf(`true_client_ip: value must only have valid characters`)
		}
	}
	if m.UserAgent != "" {
		if !norm.NFC.IsNormalString(m.UserAgent) && norm.NFD.IsNormalString(m.UserAgent) {
			// normalise NFD to NFC string
			var normErr error
			m.UserAgent, _, normErr = transform.String(transform.Chain(norm.NFD, norm.NFC), m.UserAgent)
			if normErr != nil {
				return fmt.Errorf(`user_agent: value must must be normalisable to NFC`)
			}
		}
		if strings.ContainsRune(m.UserAgent, utf8.RuneError) {
			return fmt.Errorf(`user_agent: value must must have valid encoding`)
		} else if !utf8.ValidString(m.UserAgent) {
			return fmt.Errorf(`user_agent: value must must be a valid UTF-8-encoded string`)
		}
		var _len_AuthCallbackRequest_UserAgent = len(m.UserAgent)
		if !(_len_AuthCallbackRequest_UserAgent >= 1 && _len_AuthCallbackRequest_UserAgent <= 500) {
			return fmt.Errorf(`user_agent: value must have a length between 1 and 500`)
		}
		if !_regex_b41c4bf2e7e101d6798b2df73dbde440.MatchString(m.UserAgent) {
			return fmt.Errorf(`user_agent: value must only have valid characters`)
		}
	}
	return nil
}
