...
func (m *LogOnlyValidationMessage) Validate() error {
	...
	if !proto.IsUUIDv4(m.ImageId) {
		...
		if !isValidId {
			fmt.Printf("[log-only] %s: value must %s: Base64Encoded input: %s\n", "image_id", "be parsable as UUIDv4 or legacy ID or S12 ID", proto.Base64Encode(proto.FirstCharactersFromString(m.ImageId, 50)))
		}
	}
	...
	if m.OldId != "" {
		if !norm.NFC.IsNormalString(m.OldId) && norm.NFD.IsNormalString(m.OldId) {
			// normalise NFD to NFC string
			var normErr error
			m.OldId, _, normErr = transform.String(transform.Chain(norm.NFD, norm.NFC), m.OldId)
			if normErr != nil {
				fmt.Printf("[log-only] %s: value must %s: Base64Encoded input: %s\n", "old_id", "must be normalisable to NFC", proto.Base64Encode(proto.FirstCharactersFromString(m.OldId, 50)))
			}
		}
		if strings.ContainsRune(m.OldId, utf8.RuneError) {
			fmt.Printf("[log-only] %s: value must %s: Base64Encoded input: %s\n", "old_id", "must have valid encoding", proto.Base64Encode(proto.FirstCharactersFromString(m.OldId, 50)))
		} else if !utf8.ValidString(m.OldId) {
			fmt.Printf("[log-only] %s: value must %s: Base64Encoded input: %s\n", "old_id", "must be a valid UTF-8-encoded string", proto.Base64Encode(proto.FirstCharactersFromString(m.OldId, 50)))
		}
		m.OldId = proto.RegexPua.ReplaceAllString(m.OldId, "")
		var _len_LogOnlyValidationMessage_OldId = len(m.OldId)
		if !(_len_LogOnlyValidationMessage_OldId >= 1 && _len_LogOnlyValidationMessage_OldId <= 5) {
			fmt.Printf("[log-only] %s: value must %s: Base64Encoded input: %s\n", "old_id", "have a length between 1 and 5", proto.Base64Encode(proto.FirstCharactersFromString(m.OldId, 50)))
		}
		if !_regex_b3f79e2470927c095fff6ea841e2a650.MatchString(m.OldId) {
			fmt.Printf("[log-only] %s: value must %s: Base64Encoded input: %s\n", "old_id", "only have valid characters", proto.Base64Encode(proto.FirstCharactersFromString(m.OldId, 50)))
		}
	}
	if m.TemplateRevIdOptional != "" {
		if !proto.IsTemplateRevId(m.TemplateRevIdOptional) {
			fmt.Printf("[log-only] %s: value must %s: Base64Encoded input: %s\n", "template_rev_id_optional", "must be parsable", proto.Base64Encode(proto.FirstCharactersFromString(m.TemplateRevIdOptional, 50)))
		}
	}
	if !proto.IsTemplateRevId(m.TemplateRevIdLogOnly) {
		fmt.Printf("[log-only] %s: value must %s: Base64Encoded input: %s\n", "template_rev_id_log_only", "must be parsable", proto.Base64Encode(proto.FirstCharactersFromString(m.TemplateRevIdLogOnly, 50)))
	}
	if !proto.IsTemplateRevId(m.TemplateRevId) {
		return fmt.Errorf(`template_rev_id: value must must be parsable`)
	}
	return nil
}
