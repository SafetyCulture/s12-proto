...
func (m *LogOnlyValidationMessage) Validate() error {
	...
	if !proto.IsUUIDv4(m.QuestionId) {
		...
		if !isValidId {
			fmt.Printf("[log-only] %s: value must be %s: Base64Encoded input: %s\n", "question_id", "be parsable as UUIDv4 or legacy ID", proto.Base64Encode(proto.FirstCharactersFromString(fmt.Sprintf("%v", m.QuestionId), 50)))
		}
	}
	...
	if m.Answer != "" {
		if !norm.NFC.IsNormalString(m.Answer) && norm.NFD.IsNormalString(m.Answer) {
			// normalise NFD to NFC string
			var normErr error
			m.Answer, _, normErr = transform.String(transform.Chain(norm.NFD, norm.NFC), m.Answer)
			if normErr != nil {
				fmt.Printf("[log-only] %s: value must be %s: Base64Encoded input: %s\n", "answer", "must be normalisable to NFC", proto.Base64Encode(proto.FirstCharactersFromString(fmt.Sprintf("%v", m.Answer), 50)))
			}
		}
		if strings.ContainsRune(m.Answer, utf8.RuneError) {
			fmt.Printf("[log-only] %s: value must be %s: Base64Encoded input: %s\n", "answer", "must have valid encoding", proto.Base64Encode(proto.FirstCharactersFromString(fmt.Sprintf("%v", m.Answer), 50)))
		} else if !utf8.ValidString(m.Answer) {
			fmt.Printf("[log-only] %s: value must be %s: Base64Encoded input: %s\n", "answer", "must be a valid UTF-8-encoded string", proto.Base64Encode(proto.FirstCharactersFromString(fmt.Sprintf("%v", m.Answer), 50)))
		}
		m.Answer = proto.RegexPua.ReplaceAllString(m.Answer, "")
		var _len_LogOnlyValidationMessage_Answer = len(m.Answer)
	...
	if m.Note != "" {
		if !norm.NFC.IsNormalString(m.Note) && norm.NFD.IsNormalString(m.Note) {
			// normalise NFD to NFC string
			var normErr error
			m.Note, _, normErr = transform.String(transform.Chain(norm.NFD, norm.NFC), m.Note)
			if normErr != nil {
				return fmt.Errorf(`note: value must must be normalisable to NFC`)
			}
		}
		if strings.ContainsRune(m.Note, utf8.RuneError) {
			return fmt.Errorf(`note: value must must have valid encoding`)
		} else if !utf8.ValidString(m.Note) {
			return fmt.Errorf(`note: value must must be a valid UTF-8-encoded string`)
		}
		m.Note = strings.ReplaceAll(m.Note, "\r", "")
		m.Note = proto.RegexPua.ReplaceAllString(m.Note, "")
		var _len_LogOnlyValidationMessage_Note = len(m.Note)
		if !(_len_LogOnlyValidationMessage_Note >= 5 && _len_LogOnlyValidationMessage_Note <= 40) {
			return fmt.Errorf(`note: value must have a length between 5 and 40`)
		}
		if !_regex_56b61c17208942cd7fb89ac222d5f0a2.MatchString(m.Note) {
			return fmt.Errorf(`note: value must only have valid characters`)
		}
	}
	return nil
}
