...
func (m *TestStringValue) Validate() error {
	if m.TestValue.Value != "" {
		if !norm.NFC.IsNormalString(m.TestValue.Value) && norm.NFD.IsNormalString(m.TestValue.Value) {
			// normalise NFD to NFC string
			var normErr error
			m.TestValue.Value, _, normErr = transform.String(transform.Chain(norm.NFD, norm.NFC), m.TestValue.Value)
			if normErr != nil {
				return fmt.Errorf(`test_value: value must must be normalisable to NFC`)
			}
		}
		if strings.ContainsRune(m.TestValue.Value, utf8.RuneError) {
			return fmt.Errorf(`test_value: value must must have valid encoding`)
		} else if !utf8.ValidString(m.TestValue.Value) {
			return fmt.Errorf(`test_value: value must must be a valid UTF-8-encoded string`)
		}
		var _len_TestStringValue_TestValue = len(m.TestValue.Value)
		if !(_len_TestStringValue_TestValue >= 1 && _len_TestStringValue_TestValue <= 2) {
			return fmt.Errorf(`test_value: value must have a length between 1 and 2`)
		}
		if !_regex_d4db71516b8749dc594e5bf604c6a110.MatchString(m.TestValue.Value) {
			return fmt.Errorf(`test_value: value must only have valid characters`)
		}
	}
	if len(m.TestValues) > 0 {
		for _, item := range m.TestValues {
			if item.Value != "" {
				if !norm.NFC.IsNormalString(item.Value) && norm.NFD.IsNormalString(item.Value) {
					// normalise NFD to NFC string
					var normErr error
					item.Value, _, normErr = transform.String(transform.Chain(norm.NFD, norm.NFC), item.Value)
					if normErr != nil {
						return fmt.Errorf(`test_values: value must must be normalisable to NFC`)
					}
				}
				if strings.ContainsRune(item.Value, utf8.RuneError) {
					return fmt.Errorf(`test_values: value must must have valid encoding`)
				} else if !utf8.ValidString(item.Value) {
					return fmt.Errorf(`test_values: value must must be a valid UTF-8-encoded string`)
				}
				var _len_TestStringValue_TestValues = len(item.Value)
				if !(_len_TestStringValue_TestValues >= 1 && _len_TestStringValue_TestValues <= 2) {
					return fmt.Errorf(`test_values: value must have a length between 1 and 2`)
				}
				if !_regex_d4db71516b8749dc594e5bf604c6a110.MatchString(item.Value) {
					return fmt.Errorf(`test_values: value must only have valid characters`)
				}
			}
		}
	}
