...
func (m *LogOnlyValidationMessage) Validate() error {
	...
	if m.Longitude != 0 {
		// Range check lower bounds
		if m.Longitude < -180 {
			return fmt.Errorf(`longitude: value must be greater than or equal to -180`)
		}
		// Range check upper bounds
		if m.Longitude > 180 {
			return fmt.Errorf(`longitude: value must be less than or equal to 180`)
		}
	}
	return nil
}
...
func (m *LowercaseValidationMessage) Validate() error {
	...
	if !proto.IsUUIDv4(m.Uuidv4LogOnly) {
		isValidId := false
		if !isValidId {
			fmt.Printf("[log-only] %s: value must %s: Base64Encoded input: %s\n", "uuidv4_log_only", "be parsable as UUIDv4", proto.Base64Encode(proto.FirstCharactersFromString(m.Uuidv4LogOnly, 50)))
		}
	}
	if !proto.IsUUIDv4(m.S12Id) {
		isValidId := false
	...
	if !proto.IsUUIDv4(m.S12IdLogOnly) {
		...
		if !isValidId {
			fmt.Printf("[log-only] %s: value must %s: Base64Encoded input: %s\n", "s12id_log_only", "be parsable as UUIDv4 or S12 ID", proto.Base64Encode(proto.FirstCharactersFromString(m.S12IdLogOnly, 50)))
		}
	}
	...
	if !proto.IsUUIDv4(m.LegacyLogOnly) {
		...
		if !isValidId {
			fmt.Printf("[log-only] %s: value must %s: Base64Encoded input: %s\n", "legacy_log_only", "be parsable as UUIDv4 or legacy ID", proto.Base64Encode(proto.FirstCharactersFromString(m.LegacyLogOnly, 50)))
		}
	}
