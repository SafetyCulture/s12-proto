...
const (
	...
	s12IdStrict string = "^(audit|template|action|user|ntfmsg|evidence|role|location|responseset|response|preference|heads_up|subscription)_([0-9a-fA-F]){32}$"
...
var (
	...
	rxS12IdStrict               = regexp.MustCompile(s12IdStrict)
...
// IsS12IDStrict checks if the string matches a S12ID with specific domains
func IsS12IDStrict(str string) bool {
	return rxS12IdStrict.MatchString(str)
}
