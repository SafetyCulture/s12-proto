...
const (
	...
	legacyId          string = "(?i)^[0-9a-f]{8}-?[0-9a-f]{4}-?[0-9a-f]{4}-?[0-9a-f]{4}-?[0-9a-f]{12}(-?[0-9a-f]{2,5}-?[0-9a-f]{1,16})?$"
...
var (
	...
	rxLegacyIdLowercase         = regexp.MustCompile(legacyIdLowercase)
	rxS12Id                     = regexp.MustCompile(s12Id)
	rxS12IdLowercase            = regexp.MustCompile(s12IdLowercase)
	rxLongPrefixedLegacyId      = regexp.MustCompile(longPrefixedLegacyId)
	...
	rxNonEmailUsername          = regexp.MustCompile(reNonEmailUsername)
	...
	// AIMarkBeforeDotMatcher matches a run of AI content mark carriers sitting
	// immediately in front of a dot, with the dot captured.
	//
	// That is the one placement that hides a partial URL from BreakURLMatcher,
	// which needs a word character before the dot to find a boundary. A mark
	// anywhere else in the value, including straight after the dot, leaves the
	// boundary intact. Removing only this run keeps every other copy of the mark,
	// so a field that breaks partial URLs stays marked.
	//
	// The Mitti codec inserts only at a word start, which is a non-space
	// character following whitespace, so it never produces this placement. In
	// practice this removes nothing the codec wrote and only removes a carrier
	// placed there deliberately.
	AIMarkBeforeDotMatcher = regexp.MustCompile(`[\x{2062}\x{2064}]+(\.)`)
)
...
func IsLegacyID(str string, lowerCaseOnly bool) bool {
	if lowerCaseOnly {
		return rxLegacyIdLowercase.MatchString(str)
	}
	return rxLegacyId.MatchString(str)
}

// IsS12ID checks if the string has the format of S12ID or UUID
func IsS12ID(str string, lowerCaseOnly bool) bool {
	if lowerCaseOnly {
		return rxS12IdLowercase.MatchString(str)
	}
	return rxS12Id.MatchString(str)
}

// IsLongPrefixedLegacyID checks if the string has the format of LongPrefixedLegacyID
func IsLongPrefixedLegacyID(str string) bool {
	return rxLongPrefixedLegacyId.MatchString(str)
}
...
func IsValidNonEmail(username string) bool {
	...
	return rxNonEmailUsername.MatchString(username)
}
